...
```{r}
a<-as.data.frame(table(data$OFNS_DESC)) # Count the number of crimes under the crime category
colnames(a)<-c("offense","Freq")
ggplot(a,aes(x=Freq,y=reorder(offense,Freq)))+
  geom_point()+
  xlab("crime counts")+
  ylab("offense type")
```
...
```{r}
b<-as.data.frame(table(data$PREM_TYP_DESC))
colnames(b)<-c("premise","Freq")
ggplot(b,aes(x=Freq,y=reorder(premise,Freq)))+
  geom_point()+
  ylab("crime counts")+
  xlab("premise")
```
