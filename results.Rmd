...
ggplot(x,aes(x=Freq,y=reorder(crimes,Freq),fill=level))+
...
ggplot(data, aes(x=Longitude, y=Latitude, color=BORO_NM))+
  geom_point(size=0.005, alpha=0.0075,show.legend =FALSE)+
...
library(forcats)
ggplot(tidyt,aes(x=count,y=fct_reorder2(race,type=="Victim",count,.desc=FALSE)))+
...
```
```{r}
heatmap(table(temp2$PREM_TYP_DESC,temp2$OFNS_DESC),cexCol = 0.5 )
```
...
ggplot(temp3,aes(x=Longitude, y=Latitude,fill=day))+
  geom_point(aes(color=day), size=0.05)+
  facet_wrap(~day)
```
